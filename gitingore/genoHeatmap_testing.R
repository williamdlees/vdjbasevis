...
genoHeatmap <- function(geno_table, chain = c("IGH", "IGK", "IGL"), gene_sort = "position", removeIGH = TRUE, lk_cutoff = 1, mark_low_lk = TRUE, html = FALSE, n_line = 4, line_length=60, pseudo_genes = FALSE, ORF_genes = FALSE) {
  ...
  # split rows with multiple alleles
  ...
  # Texture for low lk values
  geno_db_texture <- setNames(data.frame(matrix(ncol = 8, nrow = 0)), c("SUBJECT","GENE","ALLELES",'K','points','yend','x','xend'))
  geno_db_texture <- geno_db[K<lk_cutoff & !ALLELES %in% c("Unk", "Del", "NR")] %>%
    mutate(n_line = n_line) %>% splitstackshape::expandRows("n_line") %>%
    group_by(SUBJECT,GENE,ALLELES) %>%
    mutate(points = seq(0, 0.9, length.out = n_line),
           yend = seq(0, 0.9, length.out = n_line) + 0.1, x = gene_loc[as.character(.data$GENE)] - 0.49,
           xend = gene_loc[as.character(.data$GENE)] + 0.49)


  # sort the heatmap for plotting
  heatmap.df <- setDT(geno_db)[, n:=  .N, by = list(SUBJECT,GENE)]
  heatmap.df[, freq:= ifelse(n == 2, 0.5, ifelse(n != 1, 0.25, 1))]
  heatmap.df$title <- "Genotype"

  # change ambiguous alleles call
  id <- grepl("[0-9][0-9]_[0-9][0-9]", heatmap.df$ALLELES)
  if (any(id)) {
    num_text <- paste0('[*',1:length(unique(heatmap.df$ALLELES[id])),']')
    names(num_text) <- unique(heatmap.df$ALLELES[id])
    heatmap.df$text <- ''
    heatmap.df$text[id] <- num_text[heatmap.df$ALLELES[id]]
    heatmap.df$text_bottom <- ''
    heatmap.df$text_bottom[id] <- paste(num_text[heatmap.df$ALLELES[id]],heatmap.df$ALLELES[id])
    heatmap.df$size <- size
    heatmap.df <-  mutate(.data = heatmap.df, s = 0.5/.data$n, by = 1/.data$n)
    heatmap.df[, pos:= seq.int(s[1],1,by = by[1]) , by = list(GENE,SUBJECT)]

    heatmap.df$NEW_ALLELES <- heatmap.df$ALLELES
    heatmap.df$NEW_ALLELES[id] <- "NRA"

  }

  heatmap.df$ALLELE_TEXT <- sapply(1:nrow(heatmap.df),function(i){
    if(grepl("[0-9][0-9]_[0-9][0-9]", heatmap.df$ALLELES[i])){
      unique(non_reliable_alleles_text$text_bottom[heatmap.df$GENE[i]==non_reliable_alleles_text$GENE & grepl(heatmap.df$ALLELES[i],
