...
#'
#' @export
genoHeatmap2 <- function(geno_table, chain = c("IGH", "IGK", "IGL"), gene_sort = "position", removeIGH = TRUE, lk_cutoff = 1, mark_low_lk = TRUE, html = FALSE, n_line = 4, line_length=60, pseudo_genes = FALSE, ORF_genes = FALSE) {
...
geno_db <- sortDFByGene(DATA = genotype, chain = chain, method = gene_sort, removeIGH = removeIGH, geno = T,
...
axis(2,(0:(samples_n-1))/(samples_n-1),rownames(m),las=1,cex.axis=0.8) # bottom
